...
<body>
  <iframe src="_inc_header_php.jsp" style="width: 100%;margin-top:-76px;z-index:9999"></iframe>
<?php

// vars
$gallery_path = "/home/sites/regalamiunsorriso/www/mypics2";
...
print "<div class=boxedDark align=center>";
print "<div><a href='javascript:history.back()'>Indietro</a></div>";
if ( $_REQUEST[ 'nomedir' ] == "" ) {
	$realpath = $gallery_path;
} else {
	$realpath = $_REQUEST[ 'nomedir' ];
	$realpath = str_replace( "/mnt/nas2", "/home/sites/regalamiunsorriso/www", $realpath );
}
...
if ( count( $dirs ) != 0 ) {
	...
	foreach ( $dirs as $dir ) {
		...
		if ( $dir[ 0 ] != '_' )
			print "<a href=gallery2.php?nomedir=" . rawurlencode( realpath( $dir ) ) . "><img src=pics/dir.png /><br />$dir</a></td>";
		$index++;
		if ( $index % 4 == 0 ) {
			echo "</tr><tr>";
		}
