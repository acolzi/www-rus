...
CREATE TABLE `LOG_FOTO` (
    `id_logFoto` INTEGER NOT NULL AUTO_INCREMENT,
    `dataLog` DATE,
    `descrizione` VARCHAR(254),
    ...
    CONSTRAINT `PK_LOG_FOTO` PRIMARY KEY (`id_logFoto`)
);

CREATE INDEX `IDX_LOG_FOTO_1` ON `LOG_FOTO` (`id_users`);

# ---------------------------------------------------------------------- #
# Add foreign key constraints                                            #
# ---------------------------------------------------------------------- #
### fare a mano
#ALTER TABLE `LOG_FOTO` ADD CONSTRAINT `USERS_LOG_FOTO` 
#    FOREIGN KEY (`id_users`) REFERENCES `USERS` (`id_users`);
	
	
	
# ---------------------------------------------------------------------- #
# Alter table "LOG_FOTO"                                                 #
# ---------------------------------------------------------------------- #

ALTER TABLE `LOG_FOTO` ADD COLUMN `id_foto` INTEGER;

# ---------------------------------------------------------------------- #
# Add foreign key constraints                                            #
# ---------------------------------------------------------------------- #
## fare a mano
#ALTER TABLE `LOG_FOTO` ADD CONSTRAINT `FOTO_LOG_FOTO` 
#    FOREIGN KEY (`id_foto`) REFERENCES `FOTO` (`id_foto`);

## cambio indice sulla tabella foto

DROP INDEX `IDX_FOTO_1` ON `FOTO`;

CREATE UNIQUE INDEX `IDX_FOTO_1` ON `FOTO` (`file` ASC,`id_puntoFoto` ASC);


### copio i dati da LOG a LOG_FOTO


insert into LOG_FOTO (dataLog, descrizione, lastUpdTmst,id_users, ipAddress) select dataLog, descrizione, lastUpdTmst,id_users, ipAddress from LOG;
...
alter table USERS add column  `dataVisual` DATE;
alter table USERS add column nFotoVisualOggi integer

##drop table LOG;



CREATE INDEX `IDX_LOG_FOTO_2` ON `LOG_FOTO` (`createTmst` DESC);

CREATE INDEX `IDX_LOG_FOTO_3` ON `LOG_FOTO` (`descrizione`);
