...
CREATE TABLE `FOTO_GARA` (
    `id_fotoGara` INTEGER NOT NULL AUTO_INCREMENT,
    `id_gara` INTEGER,
    `id_puntoFoto` INTEGER,
    `fileName` VARCHAR(254),
    `tempo` TIME,
    `flgStato` INTEGER DEFAULT 0,
    `pettorali` VARCHAR(254),
    `impression` INTEGER,
    `dataUltimaVisual` DATE,
    CONSTRAINT `PK_FOTO_GARA` PRIMARY KEY (`id_fotoGara`)
);

CREATE INDEX `IDX_FOTO_GARA_1` ON `FOTO_GARA` (`id_gara`);

CREATE INDEX `IDX_FOTO_GARA_2` ON `FOTO_GARA` (`id_puntoFoto`);

# ---------------------------------------------------------------------- #
# Add foreign key constraints                                            #
# ---------------------------------------------------------------------- #

ALTER TABLE `FOTO_GARA` ADD CONSTRAINT `GARA_FOTO_GARA` 
    FOREIGN KEY (`id_gara`) REFERENCES `GARA` (`id_gara`);

ALTER TABLE `FOTO_GARA` ADD CONSTRAINT `PUNTO_FOTO_FOTO_GARA` 
